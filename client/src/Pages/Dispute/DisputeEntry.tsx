...
const DisputeEntry: React.FC<{ userprofile?: string }> = () => {
    ...
    const fatchBranch = useCallback(async () => {
        try {
            const response = await net.openRequest('/v1/oauth/branchs')
    ...
    const fatchAcquirers = useCallback(async () => {
        try {
            ...
            const response = await api.useApi('GET', '/v1/dispute/acquirers')
    ...
    const fetchAccountTitle = useCallback(async (value: any) => {
        setLoading(true);
        try {
            const res = await api.useApi('GET', `/v1/cbs/account_info/${value}`)
    ...
    const handleSubmit = async () => {
        try {
            setLoading(true);
            const res = await api.useApi('POST', '/v1/dispute/add', formData);
