...
const CustomButton: React.FC<CustomButtonProps> = ({ variant = 'regular', loading = false, text, color = "#053223", type, icon, onClick, className }) => {
    ...
    return (
        <button className={className} type={type} style={buttonStyle} onClick={onClick}>
            ...
            {icon && <span style={{ display: 'inline-block', marginRight: '8px' }}>{icon}</span>}
            {text && <span>{text}</span>}
        </button>
    );
};
